...
1. A 403 during artifact discovery usually means the step is missing
   `GITHUB_TOKEN` or the token was not made available to the job.
2. A long source compilation followed by duplicate-`cssparser` errors usually
   means `--locked` was removed or ignored.
3. A missing `src-tauri` message means a cache action is using the obsolete
