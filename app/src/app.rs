...
pub fn app() -> Html {
    ...
    spawn_local(async move {
        let mut authentication_stream = listen::<bool>("event::authentication").await.unwrap();
        while let Some(event) = authentication_stream.next().await {
            info!("Authentication status changed: {}", event.payload);
            authenticated_clone.set(event.payload);
        }
    });
